...
const ContributionDetail: React.FC = () => {
  ...
  const getRatings = async (contributionId?: number) => {
    if (!contribution && !contributionId) return
    const cid = contributionId || contribution!.id
    const { avgRating, ratingsCount } = await getRatingsByContribution(cid)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsEditing(false)
      setNewFiles([])
      setSnackbar({ open: true, message: 'Contribución actualizada', severity: 'success' })
    } catch (error) {
      console.error(error)
        ...
        <Paper elevation={2} sx={{ p: 4, mb: 3, borderRadius: 2 }}>
          ...
          {isEditing ? (
            ...
            <>
              ...
              <Typography sx={{ mb: 2 }}>{contribution.description}</Typography>
            </>
          )}
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  ...
                  {contribution.files.length > 1 && (
                    <Button
                      ...
                      sx={{
                        borderColor: getResourceTypeColor(contribution.resourceType),
                        color: getResourceTypeColor(contribution.resourceType),
                        '&:hover': {
                          borderColor: getResourceTypeColor(contribution.resourceType),
                          backgroundColor: `${getResourceTypeColor(contribution.resourceType)}10`,
                        },
                      }}
                    >
                    ...
                    >
                      ...
                      <Button
                        ...
                        sx={{
                          ...
                          flexShrink: 0,
                          ml: 2,
                          '&:hover': { backgroundColor: getResourceTypeColor(contribution.resourceType), filter: 'brightness(0.9)' },
                        }}
                      >
